...
if __name__ == '__main__':
    def modify_parser(parser):
        parser.add_argument(
            '--magnitudes', type=int, nargs='+', default=(0, 1, 2, 4, 8, 16, 32),
        )
        parser.add_argument(
        ...
        parser.set_defaults(
            comment='{obj}_cifar_multiscale_{oesamples}OE_{msm}',
            objective='hsc',
            dataset='cifar10',
    ...
    args = ms_argsparse(
        lambda s: f"{s} Repeats this whole procedure multiple times with different magnitudes for the `--ms-mode`. "
                  f"This specific script comes with a default configuration for CIFAR-10. ",
